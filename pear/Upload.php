...
//    var $MOD_MPFORM = LANGUAGE;
    ...
    function HTTP_Upload_Error($html = false)
    {
		...
		if(dirname(__FILE__).'/languages/'.LANGUAGE .'.php') {
...
***/		
    ...
    {
        if (!empty($this->error_codes[$e_code])) {
            $msg = $this->html ?
                html_entity_decode($this->error_codes[$e_code]) :
                $this->error_codes[$e_code];
        } else {
            $msg = $e_code;
        }

        if (!empty($this->error_codes['ERROR'])) {
            $error = $this->error_codes['ERROR'];
        } else {
            $error = $this->error_codes['ERROR'];
        }
        return $error.' '.$msg;
    ...
    function &raiseError($message =null,
                         ...
                         $skipmsg = false)
    {
        return PEAR::raiseError($this->errorCode($message), $message);
    }
}
...
class HTTP_Upload extends HTTP_Upload_Error
{
     ...
     * @param mixed $force_build:
     *    - force _buildFiles on each call (MUST BE true FOR MULTIPLE FILE UPLOAD)
     *
     * @return mixed array or object (see @param $file above) or Pear_Error
     * @access public
     */
    function &getFiles($file = null,$force_build = false)
    {
